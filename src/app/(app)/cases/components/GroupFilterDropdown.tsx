    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      <PopoverTrigger render={
        <Button
          ...
          variant="outline"
          size="sm"
          className="w-auto min-w-[130px] px-3 justify-between font-medium h-8 rounded-[8px] bg-white border-0 shadow-x-small py-[6px] gap-2 text-[14px] leading-5 tracking-[-0.006em] shrink-0 cursor-pointer text-[#171717] hover:bg-neutral-50 transition-all"
        >
          <span className="truncate">{selectedLabel}</span>
          <ChevronDown
            className={`size-4 shrink-0 transition-transform duration-200 ${
              open ? "rotate-180 text-[#171717]" : "text-[#5C5C5C]"
            }`}
          />
        </Button>
      } />
      ...
      >
        ...
        <div className="p-sm border-b border-neutral-100">
          <div className="relative">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 size-3.5 text-[#A4A4A4] pointer-events-none z-10" />
            <Input
              type="text"
              value={search}
